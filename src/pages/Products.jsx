    ...
    <div style={{ direction: "rtl", fontFamily: "'Segoe UI', Tahoma, sans-serif", background: "#f5f7f5", minHeight: "100vh" }}>
      ...
      <div style={{ background: `linear-gradient(135deg, ${GREEN}, #245f38)`, padding: "40px 24px" }}>
        <div style={{ maxWidth: "960px", margin: "0 auto" }}>
          <button onClick={() => navigate(-1)} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.6)", cursor: "pointer", fontSize: "14px", marginBottom: "10px", padding: 0 }}>← رجوع</button>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            <span style={{ fontSize: "36px" }}>{catInfo.icon}</span>
            <div>
              <h1 style={{ margin: 0, fontSize: "26px", color: GOLD, fontWeight: "900" }}>{catInfo.label}</h1>
              <p style={{ color: "rgba(255,255,255,0.55)", margin: "4px 0 0", fontSize: "13px" }}>{products.length} منتج متاح</p>
            </div>
          </div>
        </div>
      </div>
          ...
          {products.map(product => (
            ...
            >
              ...
              <div style={{ padding: "16px" }}>
                ...
                <p style={{ color: "#888", fontSize: "12px", margin: "0 0 12px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{product.description}</p>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ color: GOLD, fontWeight: "800", fontSize: "16px" }}>{product.price}</span>
                  <span style={{ fontSize: "12px", color: GREEN, fontWeight: "600" }}>عرض التفاصيل ←</span>
                </div>
              </div>
            </div>
          ))}
